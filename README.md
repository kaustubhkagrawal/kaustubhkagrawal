<p align="center">
  <img src="assets/banner.svg" alt="Kaustubh Agrawal - Vibe Coder, AI Builder, and AI Multi Modality" width="100%" />
</p>

<p align="center">
  <strong>I build AI-native products, developer tools, and SaaS primitives.</strong><br />
  Focused on AI multi-modality, auth/payments infrastructure, usage metering, and fast product prototyping.
</p>

---

## What I Am Building

| Project | What it is | Stack |
| --- | --- | --- |
| [better-auth-razorpay-plugin](https://github.com/kaustubhkagrawal/better-auth-razorpay-plugin) | Razorpay payments and subscriptions for Better Auth, with webhook handling and React Query helpers. | TypeScript, Better Auth, Razorpay |
| [better-auth-openmeter-plugin](https://github.com/kaustubhkagrawal/better-auth-openmeter-plugin) | OpenMeter usage metering, entitlements, API keys, organizations, and billing provider bridges for Better Auth. | TypeScript, Better Auth, OpenMeter |

## Focus

- AI product engineering and multi-modal workflows
- Developer tools and internal tooling
- Auth, billing, usage metering, and SaaS infrastructure
- Fast idea-to-product prototyping

## Activity

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=kaustubhkagrawal&theme=react-dark&bg_color=0d1117&color=58a6ff&line=58a6ff&point=f8fafc&area=true&hide_border=true" width="100%" alt="Kaustubh Agrawal's GitHub activity graph" />
</p>

## Live Activity

#### Currently working on



- [kaustubhkagrawal/better-auth-openmeter-plugin](https://github.com/kaustubhkagrawal/better-auth-openmeter-plugin) - Better Auth plugin for OpenMeter usage metering, entitlements, API keys, organizations, and billing provider bridges. (today)
- [kaustubhkagrawal/better-auth-razorpay-plugin](https://github.com/kaustubhkagrawal/better-auth-razorpay-plugin) - Better Auth plugin for Razorpay payments, subscriptions, checkout, webhooks, and React Query helpers. (1 day ago)
- [mixiopro/wavex-kalaasetu-demo](https://github.com/mixiopro/wavex-kalaasetu-demo) -  (6 months ago)


#### Recent pull requests



- [Add billing catalog control plane](https://github.com/kaustubhkagrawal/better-auth-openmeter-plugin/pull/1) on [kaustubhkagrawal/better-auth-openmeter-plugin](https://github.com/kaustubhkagrawal/better-auth-openmeter-plugin) (1 day ago)
- [[codex] add CI and release automation](https://github.com/kaustubhkagrawal/better-auth-razorpay-plugin/pull/1) on [kaustubhkagrawal/better-auth-razorpay-plugin](https://github.com/kaustubhkagrawal/better-auth-razorpay-plugin) (1 day ago)


#### Recent stars



- [nesquena/hermes-webui](https://github.com/nesquena/hermes-webui) - Hermes WebUI: The best way to use Hermes Agent from the web or from your phone! (1 day ago)
- [garrytan/gbrain](https://github.com/garrytan/gbrain) - Garry&#39;s Opinionated OpenClaw/Hermes Agent Brain (1 day ago)
- [necto-pro/pgit](https://github.com/necto-pro/pgit) -  (7 months ago)
- [kepano/defuddle](https://github.com/kepano/defuddle) - Get the main content of any page as Markdown. (9 months ago)
- [midday-ai/packrun](https://github.com/midday-ai/packrun) - npm for agents and humans (9 months ago)


## Operating Mode

- Ship useful things before they become over-designed.
- Use AI as a product and engineering multiplier.
- Keep systems small enough to understand and strong enough to run.
- Learn from real users, real payments, and real deployment constraints.

## Connect

Open to collaborations around AI products, devtools, SaaS infrastructure, auth, payments, and indie product experiments.

[GitHub](https://github.com/kaustubhkagrawal)
